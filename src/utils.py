...
def get_ner_example(row, columns):
    ...
    for i, tag in enumerate(tags):
        if tag == 0:
            if start_id != None:
                span = tokenizer.detokenize(tokens[start_id:i]) if isinstance(tokenizer,TreebankWordDetokenizer) else tokenizer.detokenize(tokens[start_id:i])
                tag_name = id_label_map[tags[start_id]].split('-')[1]
            ...
            if start_id == None:
                ...
                if tags[start_id] == tag-1:
                    ...
                    start_id = i
    if start_id != None:
        span = tokenizer.detokenize(tokens[start_id:]) if isinstance(tokenizer,TreebankWordDetokenizer) else tokenizer.detokenize(tokens[start_id:])
        tag_name = id_label_map[tags[start_id]].split('-')[1]
        example+=f"{tag_to_string[tag_name]} {tag_count[tag_name]+1}: {span}\n"
        tag_count[tag_name] += 1
...
def get_def_example(row, columns):
    example = ""
    tokenizer = TreebankWordDetokenizer()
    tokens = ast.literal_eval(row['tokens'])
    id_label_map = {
