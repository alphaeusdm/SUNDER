...
class LLMTune():
    def __init__(self):
        ...
        self.model_names = {
            'mistral': "mistralai/Mistral-{model_size}B-v0.3",
            'llama': "meta-llama/Llama-3.1-{model_size}B",
            'qwen': "Qwen/Qwen2.5-{model_size}B",
            'qwen3': "Qwen/Qwen3-{model_size}B-Base"
        }
    ...
    def load_model(self, task_name="NER", model_name='llama', model_size=8, data_size=None):
        self.model_name = self.model_names[model_name].format(model_size=model_size)
        self.save_path = f"models/{task_name}/{model_name}-{model_size}B-instruction-tuned" if not data_size else f"models/{task_name}/{model_name}-{model_size}B-instruction-tuned-{data_size}"
