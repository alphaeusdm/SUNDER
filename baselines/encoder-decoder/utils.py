    ...
    for i, tag in enumerate(tags):
        if tag == 0:
            if start_id != None:
                ...
                target+=f"{tag_to_string[tag_name]} {tag_count[tag_name]+1}: {span} | "
    ...
    for i, tag in enumerate(tags):
        if tag == 0:
            if start_id != None:
                ...
                target+=f"{tag_name} {tag_count[tag_name]+1}: {span} | "
                tag_count[tag_name] += 1
                start_id = None
            else:
                continue
...
def get_binary_list_csi(text, spans):
    tokenizer = TreebankWordTokenizer()
    tokenized_text = tokenizer.tokenize(text.replace('“', '"').replace('”', '"').replace('’',"'"))
    tokenized_spans = list(map(lambda x: tokenizer.tokenize(x.replace('“', '"').replace('”', '"').replace('’',"'")),spans))
    bin_span = [0]*len(tokenized_text)
    for span in tokenized_spans:
        span_len = len(span)
        for i in range(len(tokenized_text)-span_len+1):
            if tokenized_text[i:i+span_len]==span:
                bin_span[i:i+span_len] = [1]+[2]*(span_len-1)
    return bin_span


def get_spans_csi(row, output):
    text = row['text']
    spans = []
    for item in output:
        span = re.split(r"(Span \d+): ", item)
        span = span[1:] if len(span) > 2 else span
        if len(span) > 1:
            if 'Span' in span[0] and span[1]:
                spans.append(span[1])
    bin_span = get_binary_list_csi(text, set(spans))
    return bin_span


def get_gold_labels_csi(row, columns):
    return get_binary_list_csi(row['text'], [row['span']])


def get_binary_list_tsd(text, arguments):
    tokenizer = TreebankWordTokenizer()
    tokenized_text = tokenizer.tokenize(text.replace('“', '"').replace('”', '"').replace('’',"'"))
    tokenized_arguments = list(map(lambda x: tokenizer.tokenize(x.replace('“', '"').replace('”', '"').replace('’',"'")),arguments))
    bin_argument = [0]*len(tokenized_text)
    for argument in tokenized_arguments:
        if argument:
            span_len = len(argument)
            for i in range(len(tokenized_text)-span_len+1):
                if tokenized_text[i:i+span_len]==argument:
                    bin_argument[i:i+span_len] = [1]+[2]*(span_len-1)
    return bin_argument

def get_argument_tsd(row, spans):
    text = row['text']
    arguments = []
    for span in spans:
        span = re.split(r"(Argument \d+): ", span)
        span = span[1:] if len(span) > 2 else span
        if len(span) > 1:
            if 'Argument' in span[0] and span[1]:
                arguments.append(span[1])
    bin_argument = get_binary_list_tsd(text, set(arguments))
    return bin_argument

def get_gold_labels_tsd(row, columns):
    arguments = []
    for column in columns:
        if not pd.isna(row[column]):
            arguments.append(row[column])
    return get_binary_list_tsd(row['text'], arguments)
...
def get_gold_labels(task_name, row, columns):
    ...
    if task_name == "NER":
        ...
        bin_argument = get_gold_labels_tsd(row, columns)
    else:
        bin_argument = get_gold_labels_csi(row, columns)
...
def get_spans(task_name, row, output):
    ...
    if task_name == "NER":
        ...
        bin_argument = get_argument_tsd(row, output)
    else:
        bin_argument = get_spans_csi(row, output)
    
    return bin_argument


def get_tsd_example(row, columns):
    example = ""
    for column in columns:
        if not pd.isna(row[column]):
            example+=f"{column}: {str(row[column])}\n"
        else:
            break
    return example

def get_csi_example(row, columns):
    example = f"Span 1: {row['span']}\n"
    return example
